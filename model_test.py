...
cylinder_model = torch.hub.load('.', 'custom', path=r'C:\Users\srish\yolov5\yolov5\runs\train\clyinders-1dataset3\weights\best.pt', source='local')
...
while True:
    ...
    for _, row in cylinder_detections.iterrows():
        ...
        if label == 'gas_cylinder':
            ...
            if conf >= cylinder_confidence_threshold:
                ...
                cv2.putText(frame_resized, f'Gas Cylinder {conf:.2f}', (x1, y1 - 10),
    ...
    for c in previous_cylinders:
        ...
        cv2.putText(frame_resized, 'Gas Cylinder (held)', (x1, y1 - 10),
